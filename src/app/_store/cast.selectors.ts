...
export const getCastById = (id) =>
	createSelector(selectCasts, (allCasts) => {
...
export const selectAllCasts = createSelector(selectCasts, (castDic) => {
	if (!castDic.loaded) {
		return null;
	}
	const allCasts = [];
...
export const selectCategories = createSelector(selectCategoryState, (catState) => {
	if (!catState) {
		return [];
	}
	const cats = [];
...
export const selectAllCastsWereLoaded = createSelector(selectCastState, (castState) => castState.loaded);

export const selectAllCategoriesWereLoaded = createSelector(selectCategoryState, (catState) => {
...
export const getCastEpisodes = (castID) =>
	createSelector(selectEpisodes, (allEpisodes) => {
		const episodes = [];
...
export const selectLatestEpisodes = createSelector(selectEpisodes, (allEpisodes) => {
	const episodes = [];
	for (const id of allEpisodes.ids) {
		episodes.push(allEpisodes.entities[id]);
	}
