...
import { map, switchMap, withLatestFrom, filter, take, startWith, mergeMap } from 'rxjs/operators';
...
export class CastEffect {
	...
	castRequested$ = this.actions$
		.ofType<fromCastActions.CastRequested>(fromCastActions.CastActionTypes.CAST_REQUESTED)
		.pipe(
			withLatestFrom(this.store.pipe(select(selectors.selectAllCastsWereLoaded))),
	...
	episodesRequested$ = this.actions$.pipe(
		ofType<fromCastActions.EpisodesRequested>(fromCastActions.CastActionTypes.EPISODES_REQUESTED),
	...
	categoryRequested$ = this.actions$.pipe(
		...
		withLatestFrom(this.store.pipe(select(selectors.selectAllCategoriesWereLoaded))),
		filter(([ action, flag ]) => {
			console.log(flag);
			return !flag;
		}),
	...
	castUpdate$ = this.actions$.pipe(
		ofType<fromCastActions.CastUpdate>(fromCastActions.CastActionTypes.CAST_UPDATE),
		switchMap((action) => {
	...
	castEpisodeRequested$ = this.actions$.pipe(
		ofType<fromCastActions.CastEpisodesRequested>(fromCastActions.CastActionTypes.CAST_EPISODES_REQUESTED),
		switchMap((action) => {
	...
	mapCasts(results) {
		...
		results.hits.hits.map((itm: any) => {
			...
			casts.push({
				...
				feedURL: src.url,
				imageURL: src.image,
				lastPub: itm.inner_hits ? itm.inner_hits.episode.hits.hits[0]._source.pubDate : null,
