		...
		columns: [
			{ data: 'category', title: 'Category' },
			{
				...
				render: (val, type, row, meta) => {
					return `<strong>${val}</strong> <span class='pull-right badge badge-primary'>${row.episodeCount}</span><br><span class='pull-right'>${row.lastPub
						? row.lastPub.substring(0, 10)
						: ''}</span>`;
				}
			},
			{
